...
// This is a minimal wrapper around the existing main_windows.go functionality
// It simply exports the main function so C# can call it

// All the original structs and code from main_windows.go
...
//export EasySPF_Run
func EasySPF_Run() C.int {
	// This is the main function from main_windows.go
	...
	// Start the system tray (this will block until quit)
...
// Include all the other functions from main_windows.go
...
func handleSocks5Connection(clientConn net.Conn, sshConn *ssh.Client, config *ForwardConfig, commonConfig *CommonConfig) {
	...
	socks5Server := &socks5Server{sshConn: sshConn, config: config}
...
// Connection manager methods - simplified versions of the original
...
// SOCKS5 implementation - simplified from main_windows.go
func (s *socks5Server) handleConnection(clientConn net.Conn, commonConfig *CommonConfig) error {
	...
	// Simple implementation - just handle basic SOCKS5 without auth
	...
	var target string
	if buf[3] == 0x01 && n >= 10 { // IPv4
		target = fmt.Sprintf("%d.%d.%d.%d:%d", buf[4], buf[5], buf[6], buf[7], uint16(buf[8])<<8|uint16(buf[9]))
	} else if buf[3] == 0x03 && n >= 7 { // Domain
		domainLen := int(buf[4])
		if n >= 5+domainLen+2 {
...
func (s *reverseSocks5Server) handleConnection(clientConn net.Conn, commonConfig *CommonConfig) error {
	// Simplified reverse SOCKS5 - similar to above but connects locally
	buf := make([]byte, 256)
	n, err := clientConn.Read(buf)
	if err != nil {
		return fmt.Errorf("failed to read SOCKS5 greeting: %v", err)
	...
	localConn, err := net.DialTimeout("tcp", target, 30*time.Second)
	if err != nil {
		clientConn.Write([]byte{0x05, 0x05, 0x00, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00})
		return fmt.Errorf("failed to connect to target %s: %v", target, err)
	}
...
func main() {
	// Required for C export but won't be called when used as library
}